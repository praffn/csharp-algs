...
namespace CDS.Tests.Collections
{
	...
	{
		private ArrayList<int> CreateArrayListWithValues(params int[] values)
		{
		...
		{
			Assert.True(new ArrayList<int>().Empty);
		}
		...
		{
			Assert.Equal(0, new ArrayList<int>().Count);
		}
		...
		{
			var list = CreateArrayListWithValues(values);
		...
		{
			var list = CreateArrayListWithValues(1, 2, 3, 4);
			Assert.False(list.Empty);
			list.Clear();
		...
		{
			var list = CreateArrayListWithValues(1, 2, 3);
			list.RemoveAt(1);
		...
		{
			var list = CreateArrayListWithValues(1, 2, 3, 4, 2);
			list.Remove(2);
